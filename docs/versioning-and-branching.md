...
Vivian API 会被其他系统直接调用。任何对外 API 契约变更，无论是否向后兼容、是否已同步修改 Vivian 自带前端，都必须在实施前通知仓库维护者，并在完成影响评估后取得明确确认。未经确认不得静默修改或合入。

需要确认的范围包括但不限于：

- REST API 的路径、HTTP 方法、请求参数、请求体、响应字段、字段类型、必填性与字段语义。
- SSE 的事件名称、事件顺序、`data` 载荷结构、终止行为、断线重连与重放语义。
- WebSocket 的连接协议、客户端/服务端帧类型、字段结构、路由与关闭码。
- HTTP 状态码、错误码、错误响应结构、响应头、认证和授权行为。
- 删除、重命名或新增可能影响严格校验客户端的字段或事件。

通知维护者时必须说明：

1. 当前契约与拟修改契约的准确对比，最好提供请求/响应或事件示例。
2. 已知调用方和可能受影响的未知外部调用方。
3. 兼容性结论，包括是否属于 Breaking Change。
4. 可选的兼容方案，例如保留旧结构、增加版本化接口、特性开关或迁移窗口。
5. 调用方迁移步骤、发布时间安排和回滚方案。

如果实施过程中才发现需要改变 API 契约，应暂停相关改动，先完成上述通知与确认，不得以“内部前端已经兼容”作为继续实施的依据。获得确认后，还必须：

- 更新对应 API 文档、集成示例、自动化契约测试和 `CHANGELOG.md`。
- 对 Breaking Change 使用带 `!` 的 Conventional Commit，并重新评估语义化版本级别。
- 在合入和发布说明中记录维护者确认结果、受影响调用方及迁移/回滚安排。
...
- [ ] 检查是否涉及对外 API 契约；如涉及，已通知维护者并在实施前取得明确确认。

### 开发中

- [ ] 提交保持小而完整，信息符合 Conventional Commits。
- [ ] 新增或改变行为时同步测试和文档。
- [ ] 新发现的 API 契约变化已暂停实施并完成兼容性评估与维护者确认。
- [ ] 不在普通工作分支修改正式版本号。
...
- [ ] API 契约变更已有维护者确认记录，并已更新文档、契约测试、迁移与回滚说明。
