...
DEFAULT_REQUEST_URL = "http://10.107.98.250:9016/automation/encapsulation/queryRequestListPage"
DATE_FORMATS = (
    "%Y-%m-%d %H:%M:%S",
    "%Y-%m-%dT%H:%M:%S",
    "%Y-%m-%d",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="通过 ApiDesign 封装接口查询 ITSM 服务请求单。"
    )
    parser.add_argument(
        "--url",
        default=os.getenv("ITSM_QUERY_REQUEST_URL", DEFAULT_REQUEST_URL),
        help="服务请求查询接口地址。默认读取 ITSM_QUERY_REQUEST_URL；未设置时使用 ApiDesign 文档中的地址。",
    )
    parser.add_argument("--request-handler", "--handler", dest="request_handler", help="处理人企业微信英文名。")
    parser.add_argument("--request-owner", "--owner", dest="request_owner", help="申请人企业微信英文名。")
    parser.add_argument("--itsm-id", "--itsmId", dest="itsm_id", help="ITSM 工单 ID。")
    parser.add_argument("--request-status", "--status", dest="request_status", help="请求状态。")
    parser.add_argument("--request-title", "--title", dest="request_title", help="请求标题关键字。")
    parser.add_argument("--time-begin", dest="time_begin", help="创建时间区间开始；必须和 --time-end 成对使用。")
    parser.add_argument("--time-end", dest="time_end", help="创建时间区间结束；必须和 --time-begin 成对使用。")
    parser.add_argument("--date", dest="query_date", help="查询指定日期创建的服务请求，格式 yyyy-MM-dd。")
    parser.add_argument("--current-week", action="store_true", help="查询当前周一至周日创建的服务请求。")
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="使用 --scan-all 或本地过滤时最多扫描的页数。",
    )
    parser.add_argument(
        "--timeout",
        type=float,
...
def request_status_value(value: str | None) -> int | str | None:
    if value is None or value == "":
        return None
    return int(value) if value.isdigit() else value


def api_date(value: str | None, is_end: bool) -> str | None:
    bound = parse_query_bound(value, is_end=is_end) if value else None
    return bound.date().isoformat() if bound else None


def build_base_payload(args: argparse.Namespace) -> dict[str, Any]:
    payload = {
        "requestOwner": args.request_owner,
        "requestHandler": args.request_handler,
        "requestStatus": request_status_value(args.request_status),
        "requestTitle": args.request_title,
        "startDate": api_date(args.time_begin, is_end=False),
        "endDate": api_date(args.time_end, is_end=True),
...
def contains_text(record_value: Any, expected: str | None) -> bool:
    if not expected:
        return True
    if record_value is None:
        return False
    return expected.strip() in str(record_value)


def matches_created_time(record: dict[str, Any], args: argparse.Namespace) -> bool:
    if not args.time_begin and not args.time_end:
        return True

    created_at = parse_query_bound(record.get("requestCreatedate"), is_end=False)
    ...
    if created_at is None:
        return False
    if query_begin is not None and created_at < query_begin:
        return False
    if query_end is not None and created_at > query_end:
        return False
    return True
...
def record_matches(record: dict[str, Any], args: argparse.Namespace) -> bool:
    return all(
        [
            same_text(record.get("requestHandler"), args.request_handler),
            same_text(record.get("requestOwner"), args.request_owner),
            same_text(record.get("itsmId"), args.itsm_id),
            same_text(record.get("requestStatus"), args.request_status),
            contains_text(record.get("requestTitle"), args.request_title),
            matches_created_time(record, args),
        ]
    )
...
def build_client_filters(args: argparse.Namespace) -> dict[str, Any]:
    filters = {
        "itsmId": args.itsm_id,
        "requestStatus": args.request_status,
        "requestTitle": args.request_title,
...
def print_text(records: list[dict[str, Any]], metadata: dict[str, Any]) -> None:
    print(f"匹配到 {len(records)} 条服务请求记录。")
    print(
    ...
    for index, record in enumerate(records, start=1):
        print()
        print(f"{index}. {pick(record, 'requestTitle') or '(无标题)'}")
        print(f"   itsmId: {pick(record, 'itsmId')}")
        print(f"   状态: {pick(record, 'requestStatus')}")
        print(f"   申请人: {pick(record, 'requestOwner')}")
        print(f"   处理人: {pick(record, 'requestHandler')} {pick(record, 'requestHandlerName')}".rstrip())
        print(f"   创建时间: {pick_time(record, 'requestCreatedate')}")
        print(f"   审批完成时间: {pick_time(record, 'requestRatifyFinishTime')}")
        print(f"   截止时间: {pick_time(record, 'deadLineDate')}")
        print(f"   实际完成时间: {pick_time(record, 'requestRealEndTime')}")
        print(f"   当前处理人: {pick(record, 'currentHandlers')}")
        print(f"   流程实例ID: {pick(record, 'jbpmId')}")
        print(f"   请求描述: {pick(record, 'requestDesc')}")
