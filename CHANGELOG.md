...
- 对话侧栏支持双击重命名、单层文件夹、拖拽归类及键盘/触屏可用的“移到文件夹”菜单；文件夹与归属按用户工作区持久化。
- 定时任务执行记录表头新增当前页排序控件，默认按开始时间倒序展示。
...
- 修复执行记录受历史写入顺序影响、未严格按开始时间倒序展示的问题。
- 修复运行详情中的“查看对话”未加载并定位到关联定时任务会话的问题。
